...
class CS_Recent_Post_Carousel extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
                'cs_recent_post_carousel', esc_html__('CMS Recent Post',"wp-experts"), array('description' => esc_html__('Recent Posts Widget.', "wp-experts"),)
        );
    }
    ...
    function widget($args, $instance) {
        ...
        $title = apply_filters('widget_title', empty($instance['title']) ? esc_html__('Recent Posts', "wp-experts" ) : $instance['title'], $instance, $this->id_base);
                                ...
                                <div class="cms-recent-media">  
                                   <div class="image">
                                       <a class="post-featured-img" href="<?php the_permalink(); ?>">
                                            <?php 
                                                if(has_post_thumbnail() && !post_password_required() && !is_attachment() &&  wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()), 'wp_experts_images-sm', false)):
                                                    $class = ' has-thumbnail';
                                                    $thumbnail = get_the_post_thumbnail(get_the_ID(),'wp_experts_images-sm');
                                                else:
                                                    $class = ' no-image';
