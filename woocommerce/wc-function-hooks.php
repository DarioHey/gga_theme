...
function cms_socials_share($url, $image, $title=''){
	...
	?>	
		...
		<a target="_blank" href="https://twitter.com/home?status=<?php echo urlencode(esc_html__('Check out this article',"wp-experts"));?>:%20<?php echo urlencode($title);?>%20-%20<?php echo urlencode($url);?>"><span class="share-box"><i class="fa fa-twitter"></i></span></a>
...
if(!function_exists('cms_social_share_product')){
	function cms_social_share_product(){
    	...
    	$title = get_the_title();
    	echo '<div class="cms-product-share-wrap"><label>'.esc_html__('Share:', "wp-experts").' </label>'.cms_socials_share( get_the_permalink(),$img, $title).'</div>';
	}
}
?>